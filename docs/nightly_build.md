...
The nightly build will contain what's on develop, in release mode, for Gplay variant. It is signed using a dedicated signature, and has a dedicated appId (`im.vector.app.nightly`), so it can be installed along with the production version of Mana Android. The only other difference compared to Mana Android is a different app icon background. We do not want to change the app name since it will also affect some strings in the app, and we do want to do that.

Nightly builds are built and released to Firebase every days, and automatically.

This is recommended to exclusively use this app, with your main account, instead of Mana Android, and fallback to Mana Android just in case of regression, to discover as soon as possible any regression, and report it to the team. To avoid double notification, you may want to disable the notification from the Mana Android production version. Just open Mana Android, navigate to `Settings/Notifications` and uncheck `Enable notifications for this session`.
...
```
signing.mana.nightly.storePassword=VALUE_FROM_PASSBOLT
signing.mana.nightly.keyId=VALUE_FROM_PASSBOLT
signing.mana.nightly.keyPassword=VALUE_FROM_PASSBOLT
```
