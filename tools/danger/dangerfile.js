...
if (requiresChangelog) {
    ...
    if (changelogFiles.length == 0) {
        warn("Please add a changelog. See instructions [here](https://github.com/mana-hq/mana-android/blob/develop/CONTRIBUTING.md#changelog)")
    } else {
        ...
        if (!changelogFiles.every(file => validTowncrierExtensions.includes(file.split(".").pop()))) {
            fail("Invalid extension for changelog. See instructions [here](https://github.com/mana-hq/mana-android/blob/develop/CONTRIBUTING.md#changelog)")
        }
    }
}
...
if (user != "RiotTranslateBot") {
   if (editedFiles.some(file => file.endsWith("strings.xml") && !file.endsWith("values/strings.xml"))) {
       fail("Some translation files have been edited. Only user `RiotTranslateBot` (i.e. translations coming from Weblate) is allowed to do that.\nPlease read more about translations management [in the doc](https://github.com/mana-hq/mana-android/blob/develop/CONTRIBUTING.md#internationalisation).")
   }
}
